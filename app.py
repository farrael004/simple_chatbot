...
def call_gemini(messages_history):
    ...
    for message in messages_history:
        ...
        if role not in ["user", "model"]:
            print(f"Warning: Unexpected role '{role}' found in history. Mapping to 'user'.")
...
with st.sidebar:
    ...
    "Select Gemini Model",
    ["gemini-2.5-flash-lite-preview-06-17", "gemini-2.5-flash-preview-04-17", "gemini-2.5-pro-preview-05-06"], # Add more models as needed
    index=1
    )
...
if prompt := st.chat_input("What is up?"):
    ...
    with st.chat_message("assistant"):
        if st.secrets.get("GEMINI_API_KEY"): # Only proceed if API key is set
            ...
            if streamed_response_generator: # Check if call_gemini was successful
                try:
                    ...
                    st.session_state.messages.append(Message(role="model", content=full_response_content))
                except Exception as e:
                    st.error(f"Error during streaming: {e}")
                    # Potentially add a placeholder message to history if streaming fails mid-way
                    st.session_state.messages.append(Message(role="model", content="*An error occurred while generating the response.*"))
            else:
                ...
                st.session_state.messages.append(Message(role="model", content="*Could not connect to the API.*"))

        else:
            st.error("GEMINI_API_KEY is not configured. Please add it to your Streamlit secrets.")
            st.session_state.messages.append(Message(role="model", content="*API key not configured.*"))